...
def test_target_object_transformation_with_real_rule(dummy_pipeline):
    from sigma.collection import SigmaCollection
    from sigma.processing.pipeline import ProcessingPipeline
    from sigma.processing.transformations.interim import TargetObjectTransformation
    from sigma.backends.siem.siem import SiemBackend
...
author: Florian Roth (Nextron Systems), wagga, Nasreddine Bencherchali (Splunk STRM)
date: 2018-03-20
modified: 2024-12-03
tags:
...
"""
    ...
    pipeline = ProcessingPipeline(
        items=[
            ...
            ProcessingItem(
                transformation=FieldMappingTransformation(SiemBackend.field_mappings)
            ),
        ]
    )
    ...
    # Check selection_value_lmcompatibilitylevel
    lm_level_detection = rule.detection.detections["selection_value_lmcompatibilitylevel"]
    assert len(lm_level_detection.detection_items) == 2
    target_object_item = lm_level_detection.detection_items[0]
    assert isinstance(target_object_item, SigmaDetection)
    assert target_object_item.detection_items[0].field == "ObjectName"
    assert target_object_item.detection_items[1].field == "ObjectValue"
    assert target_object_item.detection_items[1].value[0] == SigmaString("lmcompatibilitylevel")

    # Check selection_value_ntlmminclientsec
    ntlm_min_client_sec_detection = rule.detection.detections["selection_value_ntlmminclientsec"]
    assert len(ntlm_min_client_sec_detection.detection_items) == 2
    target_object_item = ntlm_min_client_sec_detection.detection_items[0]
    assert isinstance(target_object_item, SigmaDetection)
    assert target_object_item.detection_items[0].field == "ObjectName"
    assert target_object_item.detection_items[1].field == "ObjectValue"
    assert target_object_item.detection_items[1].value[0] == SigmaString("NtlmMinClientSec")
